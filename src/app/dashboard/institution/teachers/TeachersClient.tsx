// src/app/dashboard/institution/teachers/TeachersClient.tsx
'use client';
...
export function TeachersClient({ institutionId }: { institutionId: string }) {
  ...
  const {
    ...
    realtime: true,
    filters: [
      { column: 'role', operator: 'eq' as const, value: 'teacher' },
    ...
    try {
      if (editingTeacher) {
        ...
        const response = await fetch('/api/admin/users', {
          ...
          body: JSON.stringify({
            action: 'create',
            data: { ...data, role: 'teacher', institutionId },
          }),
        });
  ...
  const handleExport = () => {
    const csv = [
      ['Name', 'Email', 'Experience', 'Status', 'Joined'],
      ...teachers.map((t) => [
        t.display_name,
        t.email,
    ...
    <div className="space-y-6">
      ...
      >
        {[
          ...
          { label: 'With Subjects', value: teachers.filter((t) => t.subjects && t.subjects.length > 0).length },
